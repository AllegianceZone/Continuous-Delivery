...
if ($la ne "$ts $files[0]") {
	my $cmd = "\"$szp\" a -t7z $azbp\\Package\\Minimal_${build}.7z $azbp\\Package\\Artwork_minimal\\ -xr!*.git -mx8";
	system($cmd);
} else {
...
if ($la ne "$ts $files[0]") {
	my $cmd = "\"$szp\" a -t7z $azbp\\Package\\Hires_${build}.7z $azbp\\Package\\Artwork_detailed\\ -xr!*.git -mx8";
	system($cmd);
} else {
...
if ($la ne "$ts $files[0]") {
	my $cmd = "\"$szp\" a -t7z $azbp\\Package\\Regular_${build}.7z $azbp\\Package\\Artwork\\ -xr!*.git -mx8";
	system($cmd);
} else {
