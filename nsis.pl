...
my $bbeta = "";
my $dlcode = "";
my $clientbinary = "Allegiance.exe";
my $url = "http://cdn.allegiancezone.com/install"; #can be FTP:// 
...
if ($ver eq "1.3") {
	...
	$cfgfile = "http://autoupdate.allegiancezone.com/config/AZDev.cfg";
}
...
pdbredl:
  ...
  goto DL
  dontDL2:
	 File "C:\\build\\betareghlp.exe"
	tryagain:
	 LogEx::Write true true "Browse for Folder - Set $ver ArtPath"
	ExecWait '"betareghlp.exe"' \$BetaSetupError
