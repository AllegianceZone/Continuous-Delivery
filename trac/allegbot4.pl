...
sub doIdle {
    ...
    location /latest.exe {
      rewrite ^ http://cdn.allegiancezone.com/install/Beta_b${build}_$rev.exe permanent;
						...
						DiscourseBuildPost("New update delivered! (build $build)","deployed-b{$build}_$rev","The Allegiance Zone continuous delivery system has published a new version of the game!  Get it here: http://cdn.allegiancezone.com/install/Beta_b${build}_$rev.exe","$tracurl/build/Allegiance/$build");
					}
				} else {
